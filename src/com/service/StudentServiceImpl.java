...
public class StudentServiceImpl implements StudentService{
	...
	public boolean allowLogin(String studentID, String password) {
		//password = MD5.Encrypt(password);//MD5解密验证登录
		Student student = studentDao.findByStudenID(studentID);//根据ID查找学生的记录
		if(student==null){//如果找不到的话就返回false
			return false;
		}else {
			if(password.equals(student.getPassword())||MD5.Encrypt(password).equals(student.getPassword())){//如果传入的密码和学生的密码一致的话，就返回true
				return true;
			}else {
				return false;
			}
